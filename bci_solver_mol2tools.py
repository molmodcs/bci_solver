...
def find_duplicates(directory):
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for filename in files:
            ...
            item_counts.setdefault(base_name, 0)
            item_counts[base_name] += 1
...
def create_new_name(f_name, count):

    base_name, extension = os.path.splitext(f_name)

    new_f_name = f"{base_name} ({count}){extension}"
    ...
    with open(mol2_file_path, 'r') as f:
        ...
        for line in lines:
            if line.find(word2) != -1 :
                ...
                break
        lines_to_iterate = list(filter(lambda line : lines.index(line) > index_for_bonds, lines))
...
def charges2mol2(mol2_file_path,charges_path,output_folder=None):
    mol2_file_path = os.path.abspath(mol2_file_path)
    charges_path = os.path.abspath(charges_path)
    ...
    mol2_file_path_copy_name = os.path.splitext(os.path.basename(mol2_file_path))[0] + '_charges'

    if mol2_file_path_copy_name in duplicates:
        count = duplicates[mol2_file_path_copy_name]
        mol2_file_path_copy_name = create_new_name(mol2_file_path_copy_name,count) + ".mol2"
        mol2_file_path_copy = os.path.join(output_folder,mol2_file_path_copy_name)
    else:
        mol2_file_path_copy = os.path.join(output_folder,mol2_file_path_copy_name + ".mol2")
            ...
            for line in rf.readlines():
                if counter1 not in indices_for_atoms:
                    ...
                    if float(original_charge) < 0 and float(new_charge) < 0:
                        original_charge = original_charge[1:]
...
def charges_log2mol2(mol2_file_path, charges_log_path,output_folder=None):
    mol2_file_path = os.path.abspath(mol2_file_path)
    charges_log_path =  os.path.abspath(charges_log_path)
    if type(output_folder) == type(None):
        if not os.path.exists(r"Output"):
            os.mkdir(r"Output")
        output_folder = os.path.abspath(r"Output") 
    ...
    with open(mol2_file_path,'r') as f:
        ...
        mol2_file_path_copy_name = os.path.splitext(os.path.basename(mol2_file_path))[0] + '_charges_log'
...
def dataframe2mol2(mol2_file_path,charges_dataframe,sheet=None,output_folder=None):
    mol2_file_path = os.path.abspath(mol2_file_path)
    charges_dataframe = os.path.abspath(charges_dataframe)
    if type(output_folder) == type(None):
        if not os.path.exists(r"Output"):
            os.mkdir(r"Output")
        output_folder = os.path.abspath(r"Output")
    ...
    with open(mol2_file_path,'r') as f:
        ...
        mol2_file_path_copy_name = os.path.splitext(os.path.basename(mol2_file_path))[0] + '_dataframe'
...
def charges2mol2_folder(mol2_folder_path,charges_folder_path,output_folder=None):
    ...
    mol2_folder_path = os.path.abspath(mol2_folder_path)
    charges_folder_path = os.path.abspath(charges_folder_path)
